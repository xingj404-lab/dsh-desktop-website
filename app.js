(function () {
  ...
  var T = {
    zh: {
      ...
      nav_stack: "技术栈",
      eyebrow_stack: "技术栈",
      stack_title: "更轻、更快 —— 我们选择 Tauri，而非 Electron",
      stack_sub:
        "市面上很多桌面 AI 助手都基于 Electron 打包。dsh-desktop 改用 <strong>Tauri v2</strong>（Rust）与<strong>系统原生 WebView</strong> 构建，体积更小、占用更低、启动更快。",
      layer1_title: "系统原生 WebView",
      layer1_desc:
        "macOS 使用 WKWebView、Windows 使用 WebView2 —— 由操作系统提供并持续更新安全补丁，无需为每个应用打包一份 Chromium。",
      layer1_tag: "零额外体积",
      layer2_title: "Tauri v2（Rust）",
      layer2_desc:
        "原生窗口、菜单栏、系统托盘、自动更新等能力，全部由体积仅约 8 MB 的 Rust 内核驱动。",
      layer2_tag: "内存安全",
      layer3_title: "Node.js 运行时（内置）",
      layer3_desc:
        "随安装包内置 Node.js v22，用于运行 dsh —— 你无需自行安装 Node、npm 或任何命令行工具。",
      layer3_tag: "开箱即用",
      layer4_title: "dsh（DeepSeek Harness）",
      layer4_desc:
        "编程智能体核心，负责模型调用、代码执行与任务编排，运行在你本机的 127.0.0.1 回环地址上。",
      layer4_tag: "本地优先",
      stack_col_tauri: "Tauri v2 <code>dsh-desktop</code>",
      stack_col_electron: "Electron",
      s1_label: "渲染引擎",
      s1_tauri: "系统原生 WebView（WebKit / WebView2）",
      s1_electron: "每个应用打包一份 Chromium",
      s2_label: "框架体积",
      s2_tauri: "Rust 内核约 8 MB",
      s2_electron: "Chromium 约 100–150 MB",
      s3_label: "内存占用",
      s3_tauri: "复用系统 WebView，更低",
      s3_electron: "独立 Chromium 多进程，更高",
      s4_label: "启动速度",
      s4_tauri: "无浏览器冷启动，更快",
      s4_electron: "需加载 Chromium，较慢",
      s5_label: "后端语言",
      s5_tauri: "Rust（内存安全、高性能）",
      s5_electron: "JavaScript / Node.js",
      s6_label: "安全补丁",
      s6_tauri: "WebView 补丁由操作系统推送",
      s6_electron: "依赖应用自行升级 Chromium",
    ...
    en: {
      ...
      nav_stack: "Tech",
      eyebrow_stack: "Tech stack",
      stack_title: "Lighter and faster — we chose Tauri over Electron",
      stack_sub:
        "Most desktop AI assistants ship on Electron. dsh-desktop is built with <strong>Tauri v2</strong> (Rust) and the <strong>system WebView</strong> instead — smaller, lighter, and faster to start.",
      layer1_title: "System WebView",
      layer1_desc:
        "WKWebView on macOS and WebView2 on Windows — provided and security-patched by the OS, with no bundled Chromium per app.",
      layer1_tag: "Zero extra weight",
      layer2_title: "Tauri v2 (Rust)",
      layer2_desc:
        "Native window, menu bar, system tray, and auto-update — all driven by a Rust core of only about 8 MB.",
      layer2_tag: "Memory-safe",
      layer3_title: "Node.js runtime (bundled)",
      layer3_desc:
        "Node.js v22 ships with the installer to run dsh — no need to install Node, npm, or any CLI yourself.",
      layer3_tag: "Zero setup",
      layer4_title: "dsh (DeepSeek Harness)",
      layer4_desc:
        "The coding-agent core: model calls, code execution, and task orchestration — all on your local 127.0.0.1 loopback.",
      layer4_tag: "Local-first",
      stack_col_tauri: "Tauri v2 <code>dsh-desktop</code>",
      stack_col_electron: "Electron",
      s1_label: "Rendering engine",
      s1_tauri: "System WebView (WebKit / WebView2)",
      s1_electron: "A full Chromium per app",
      s2_label: "Framework size",
      s2_tauri: "~8 MB Rust core",
      s2_electron: "~100–150 MB Chromium",
      s3_label: "Memory use",
      s3_tauri: "Reuses the system WebView — lower",
      s3_electron: "Separate Chromium processes — higher",
      s4_label: "Startup speed",
      s4_tauri: "No browser cold start — faster",
      s4_electron: "Loads Chromium — slower",
      s5_label: "Backend language",
      s5_tauri: "Rust (memory-safe, fast)",
      s5_electron: "JavaScript / Node.js",
      s6_label: "Security patches",
      s6_tauri: "WebView patched by the OS",
      s6_electron: "App must upgrade Chromium itself",
